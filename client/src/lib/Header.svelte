...
<header>
  ...
  <div class="header-right">
    <a href="/products">
      ...
      <svg fill="none" height="18" stroke="#151515" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" viewBox="0 0 45 18" width="45"
           xmlns="http://www.w3.org/2000/svg">
        <path
            d="M38.1848 16.9977C38.1848 16.9977 42.267 17.1463 43.3666 15.0305C44.8291 12.2108 40.6776 6.1795 36.7171 4.73173C37.1363 5.81071 38.187 8.79371 37.1363 10.1161C36.8024 6.07413 36.1112 2.52951 27.8447 1.31988C27.72 1.57908 29.9269 3.98572 29.6283 6.33229C27.8447 1.89519 25.5483 0.097595 17.0929 1.43051C18.8593 2.74763 20.231 4.73173 20.231 6.38286C17.8492 2.57482 15.6028 0.179782 7.96458 4.68642C9.8952 5.74959 11.0312 7.71683 11.2989 9.00655C7.75765 4.70644 6.3262 5.0647 2.3508 9.12668C1.07083 10.7873 0.496976 12.6239 1.54122 14.7881C2.60147 16.9862 6.16621 16.7901 6.707 16.7901C9.44722 16.7523 18.9062 16.8017 38.1848 16.9977Z"
        />
      </svg>
    </a>
    {#if connected}
      <a class="cart" href={productAdded ? "" : "/cart"}>
        Panier
        ...
        {#if $cart.reduce((acc, cur) => acc + cur.quantity, 0) > 0}
          <span class="cart-count">{$cart.reduce((acc, cur) => acc + cur.quantity, 0)}</span>
        {/if}
        {#if productAdded}
          <div class="cart-popup">
            <h2>Merci !</h2>
            <span>Ce produit à été ajouté au panier</span>
            <Btn href="/cart" onClick={()=>{ productAdded = false} } small>Voir mon panier</Btn>
            <Btn onClick={(e) => {
							e.preventDefault()
							productAdded = false
						}} small outline>Continuer mes achats
            </Btn>
          </div>
        {/if}
      </a>
